...
class DelphesProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        #electrons
        ele_sel = (events.ElectronLoose.PT>10)
        electron = get_four_vec_fromPtEtaPhiM(
            None,
            pt = events.ElectronLoose.PT[ele_sel],
            eta = events.ElectronLoose.Eta[ele_sel],
            phi = events.ElectronLoose.Phi[ele_sel],
            M = np.zeros_like(events.ElectronLoose.PT[ele_sel]),
            copy = False,
        )
        electron['iso'] = events.ElectronLoose.IsolationVar[ele_sel]

        ele_l = electron[((electron['iso']<0.3)&(electron.pt>10)&(np.abs(electron.eta)<3))]

        #muons
        mu_sel = (events.MuonLoose.PT>4)
        muon = get_four_vec_fromPtEtaPhiM(
            None,
            pt = events.MuonLoose.PT[mu_sel],
            eta = events.MuonLoose.Eta[mu_sel],
            phi = events.MuonLoose.Phi[mu_sel],
            M = np.zeros_like(events.MuonLoose.PT[mu_sel]),
            copy = False,
        )
        muon['iso'] = events.MuonLoose.IsolationVar[mu_sel]

        muon_l = muon[((muon['iso']<0.25)&(muon.pt>4)&(np.abs(muon.eta)<2.8))]
        ...
        for var in variations:
            #jets
            old_jet = getJets(events, jes_corrector, '', delphes=True)  #getJets uses JetPUPPILoose for delphes samples

            jet_px_old = old_jet.pt*np.cos(old_jet.phi)
            jet_py_old = old_jet.pt*np.sin(old_jet.phi)

            jet = getJets(events, jes_corrector, var, delphes=True)   #getJets uses JetPUPPILoose for delphes samples
...
if __name__ == '__main__':
    ...
    if args.run_delphes:
        ...
        fileset_all = {
            ...
            'signal': {
                #m_a=150
                #'2HDMa_bb_sinp_0.35_tanb_1.0_mXd_10_MH3_1000_MH4_150_MH2_1000_MHC_1000': samples['2HDMa_bb_sinp_0.35_tanb_1.0_mXd_10_MH3_1000_MH4_150_MH2_1000_MHC_1000']['delphes'],
                #'2HDMa_gg_sinp_0.35_tanb_1.0_mXd_10_MH3_1000_MH4_150_MH2_1000_MHC_1000': samples['2HDMa_gg_sinp_0.35_tanb_1.0_mXd_10_MH3_1000_MH4_150_MH2_1000_MHC_1000']['delphes'],
                #'2HDMa_bb_sinp_0.35_tanb_1.0_mXd_10_MH3_1500_MH4_150_MH2_1500_MHC_1500': samples['2HDMa_bb_sinp_0.35_tanb_1.0_mXd_10_MH3_1500_MH4_150_MH2_1500_MHC_1500']['delphes'],
                #'2HDMa_gg_sinp_0.35_tanb_1.0_mXd_10_MH3_1500_MH4_150_MH2_1500_MHC_1500': samples['2HDMa_gg_sinp_0.35_tanb_1.0_mXd_10_MH3_1500_MH4_150_MH2_1500_MHC_1500']['delphes'],
                #m_a=750
